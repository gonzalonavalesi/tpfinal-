...
public class SistemaDeGestion {
    ...
    public boolean agregarProductoAlCarrito(int id, String nombreUsuario)throws FaltaStockEx {
        for(Usuario usuarios : this.arregloUsuarios){
                ...
                for(Producto producto : this.productoHashSet){
                    if(producto.getIdProducto() == id){
                        if(producto.getStock()>1){
                            ...
                            throw new FaltaStockEx("Stock insuficiente");
                        }
    ...
    public boolean agregarAlmacenamiento(int stock, double precio, String marca, String modelo, TipoAlmacenamiento tipoAlmacenamiento, int gb){
        ...
        return productoHashSet.add(new Almacenamiento(stock, precio, marca, modelo, tipoAlmacenamiento, gb));
    }

    public boolean agregarCpu(int stock, double precio, String marca, String modelo, Socket socket, int nucleos, int hilos, String integrados){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto.getModelo().equalsIgnoreCase(modelo)){
        ...
        return productoHashSet.add(new Cpu(stock, precio, marca, modelo, socket, nucleos, hilos, integrados));
    }

    public boolean agregarGpu(int stock, double precio, String marca, String modelo, int memoria){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto.getModelo().equalsIgnoreCase(modelo)){
        ...
        return productoHashSet.add(new Gpu(stock, precio, marca, modelo, memoria));
    }

    public boolean agregarMotherBoard(int stock, double precio, String marca, String modelo, Socket socket){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto.getModelo().equalsIgnoreCase(modelo)){
        ...
        return productoHashSet.add(new MotherBoard(stock, precio, marca, modelo, socket));
    }

    public boolean agregarRam(int stock, double precio, String marca, String modelo, TipoMemoria tipoMemoria, int gb){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto.getModelo().equalsIgnoreCase(modelo)){
        ...
        return productoHashSet.add(new Ram(stock, precio, marca, modelo, tipoMemoria, gb));
    }

    public boolean agregarRefrigeracion(int stock, double precio, String marca, String modelo, Socket socket, TipoRefrigeracion refrigeracion){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto.getModelo().equalsIgnoreCase(modelo)){
        ...
        return productoHashSet.add(new Refrigeracion(stock, precio, marca, modelo, socket, refrigeracion));
    }

    public boolean agregarFuente(int stock, double precio, String marca, String modelo, int watts, Certificacion certificacion){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto.getModelo().equalsIgnoreCase(modelo)){
        ...
        return productoHashSet.add(new Fuente(stock, precio, marca, modelo, watts, certificacion));
    }

    public boolean agregarGabinete(int stock, boolean rgb, double precio, String marca, String modelo){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto.getModelo().equalsIgnoreCase(modelo)){
        ...
        return productoHashSet.add(new Gabinete(stock, rgb, precio, marca, modelo));
    }

    public boolean agregarMouse(int stock, double precio, String marca, String modelo, String color, boolean rgb, boolean inalambrico, ArrayList<Integer> dpi){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
        ...
        return productoHashSet.add(new Mouse(stock, precio, marca, modelo, color, rgb, inalambrico, dpi));
    }

    public boolean agregarTeclado(int stock, double precio, String marca, String modelo, String color, boolean rgb, boolean inalambrico, TipoTeclado tipoTeclado, TipoMecanismo tipoMecanismo){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto instanceof Periferico){
        ...
        return productoHashSet.add(new Teclado(stock, precio, marca, modelo, color, rgb, inalambrico, tipoTeclado, tipoMecanismo));
    }

    public boolean agregarAuricular(int stock, double precio, String marca, String modelo, String color, boolean rgb, boolean inalambrico, String audio, boolean microfono){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto instanceof Periferico){
        ...
        return productoHashSet.add(new Auricular(stock, precio, marca, modelo, color, rgb, inalambrico, audio, microfono));
    }

    public boolean agregarMonitor(int stock, double precio, String marca, String modelo, String color, boolean rgb, boolean inalambrico, double pulgadas, String calidadPantalla, boolean curvo, int frecuencia, int tiempoRespuesta){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto instanceof Periferico){
        ...
        return productoHashSet.add(new Monitor(stock, precio, marca, modelo, color, rgb, inalambrico, pulgadas, calidadPantalla, curvo, frecuencia, tiempoRespuesta));
    }

    public boolean agregarMousePad(int stock, double precio, String marca, String modelo, String color, boolean rgb, boolean inalambrico, String tipoSuperficie, String tamaño){
        Iterator<Producto> iterator = productoHashSet.iterator();
        while(iterator.hasNext()){
            Producto producto = iterator.next();
            if(producto instanceof Periferico){
        ...
        return productoHashSet.add(new MousePad(stock, precio, marca, modelo, color, rgb, inalambrico, tipoSuperficie, tamaño));
    }
    ...
    public Ticket realizarCompra(String nombreUsuario) throws ExceptionCarritoIsNULL, ExceptionMetodoDePagoNotFound {
        Iterator<Usuario>iterator=arregloUsuarios.iterator();
        while (iterator.hasNext()){
            Usuario usuario=iterator.next();
            if(usuario.getNombre().equalsIgnoreCase(nombreUsuario)){
                ...
                for(Producto productoCarrito : usuario.getCarritoProductos().getCarrito()){
                    for(Producto productoStock : this.productoHashSet){
                        if(productoStock.getIdProducto() == productoCarrito.getIdProducto()){
                            productoStock.reducirStock();
                            break;
                        }
                    }
                }

                Ticket ticket = new Ticket(nombreUsuario);
    ...
    //AGREGAR NOTEBOOK
    public boolean agregarNotebook(int stock, double precio, String marca, String modelo, double precioCPU, String marcaCPU, String modeloCPU, Socket socket, int nucleos, int hilos, String integrados, double precioGPU, String marcaGPU, String modeloGPU, int memoriaGPU, double precioAlmacenamiento, String marcaAlmacenamiento, String modeloAlmacenamiento, TipoAlmacenamiento tipoAlmacenamiento, int gbAlmacenamiento, double precioRam, String marcaRam, String modeloRam, TipoMemoria tipoMemoria, int gbRam, int bateria, String calidadPantalla, double pulgadas, double precioMother, String marcaMother, String modeloMother, Socket socketMother) {
        Notebook notebook = new Notebook(stock, precio, marca, modelo, precioCPU, marcaCPU, modeloCPU, socket, nucleos, hilos, integrados, precioGPU, marcaGPU, modeloGPU, memoriaGPU, precioAlmacenamiento, marcaAlmacenamiento, modeloAlmacenamiento, tipoAlmacenamiento, gbAlmacenamiento, precioRam, marcaRam, modeloRam, tipoMemoria, gbRam, bateria, calidadPantalla, pulgadas, precioMother, marcaMother, modeloMother, socketMother);
